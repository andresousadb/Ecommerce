    ...
    if request.method == 'POST':
        try:
            ...
            if form.is_valid():
                ...
                review_text = form.cleaned_data['review'].strip()
                if not review_text:
                    messages.info(request, 'A avaliação está vazia, não foi adicionada.')
                    return redirect(url)
                data.save()
                messages.success(request, 'Avaliação atualizada')
            else:
                messages.error(request, 'no formulário. Por favor, preencha todos os campos corretamente.')
            return redirect(url)
